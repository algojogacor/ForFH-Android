...
| minSdk / targetSdk | **26** (Android 8.0) / **35** | Jangkau semua HP teman; izin alarm exact & full-screen intent makin ketat tapi tersedia |
...
- Android 16 (API 36) di device target; minSdk 26.
...
`session_cookie` · `alarm_offsets` (3j / 2j / 1j boolean) · `last_sync_at` · `last_sync_status` (ok/error)
...
3. Schedule via `AlarmScheduler` (exact/fallback), PendingIntent `FLAG_IMMUTABLE`, requestCode `stableHash(id)`.
...
- Tersedia → `setExactAndAllowWhileIdle()`.
- Tidak tersedia → `setWindow()` (toleransi window minimal ±10 menit pada Android modern — **jangan ditulis sebagai "±10 menit" di UX**; di Pengaturan ditulis halus: *"waktu alarm dapat sedikit bergeser"*).
- Dengar `ACTION_SCHEDULE_EXACT_ALARM_PERMISSION_STATE_CHANGED` → begitu access tersedia kembali, `rescheduleAll()` (alarm kembali exact).
- Access dicabut setelah pernah diberi → receiver tetap jalan (PendingIntent terpasang tak ikut dicabut); reschedule berikutnya pakai fallback otomatis.
...
- `NotificationManager.canUseFullScreenIntent()` → tersedia: **FullScreenAlarmActivity** di atas lock screen — nama MK, ruang, jam mulai, tombol **"Tidur lagi 3 menit"** + **"Tutup"**.
...
- Channel: `Alarm Kuliah` — importance `HIGH`, sound + vibration.
...
- Tiap snooze: +3 menit (`triggerAtMillis += 180_000`), `snoozeCount++`, reschedule exact/fallback sesuai availability, update row.
...
- Safety net: `AlarmRescheduler.reconcile()` (ikut sync WorkManager 6 jam) memastikan 3 slot selalu terpasang — kalau reschedule di receiver gagal, alarm besok tetap dipasang oleh sync berikutnya.
...
- `BOOT_COMPLETED` (receiver, izin `RECEIVE_BOOT_COMPLETED`) → `AlarmRescheduler.reconcile()` (Room dibaca setelah unlock).
...
- Wipe-and-replace **hanya** saat response sukses & valid (§10).
...
| Izin notifikasi ditolak | App tetap dipakai; Pengaturan tampil status + tombol buka system Settings. |
...
- **ReceiverGuard**: `enabled=false` → skip; `now >= startDateTime` → skip; `isLoggedIn=false` → skip; row hilang → skip.
